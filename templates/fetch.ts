...
export function patchFetch(): void {
  const alreadyPatched =
    (globalThis.fetch as Fetch)[patchFlagSymbol] || (globalThis as any)[patchFlagSymbol];
...
function applyPatch() {
  ...
  globalThis.fetch = async (...args) => {
    const request = new globalThis.Request(...args);
...
async function handleInlineAssetRequest(request: Request) {
  if (request.url.startsWith('blob:')) {
    try {
      ...
      const binaryContent = (await import(`./__next-on-pages-dist__/assets/${url.pathname}.bin`))
        .default;
 ...
 */
function setRequestUserAgentIfNeeded(request: Request<unknown, RequestInitCfProperties>): void {
